...
- انتقال مستقیم status و بدنه خطای upstream به مصرف‌کننده، همراه لاگ متادیتای امن بدون ذخیره بدنه درخواست یا پاسخ
- پنل حرفه‌ای RTL، حالت تاریک/روشن، نمودارهای مصرف و هزینه، کاتالوگ و آزمایشگاه مدل
- ArkaChat مستقل با ورود OTP موبایل، تاریخچه محلی گفتگو، انتخاب مدل فعال و مصرف امن از کیف پول و کلید AiBus
- گزارش هوش تجاری با دیت‌پیکر شمسی/میلادی، نمودار ترکیبی توکن و هزینه USD، ریزمصرف صفحه‌بندی‌شده و خروجی CSV فیلترشده
...
- گزارش CSV کاربران، لاگ مدیریتی Gateway و صفحه «لاگ درخواست‌های من» برای هر کاربر با جداسازی کامل داده‌ها، فیلتر کلید/مدل/وضعیت، HTTP status و latency
...
برای راه‌اندازی اولیه‌ی محلی Production می‌توان `EXPOSE_SUPERADMIN_OTP=true` را موقتاً در `.env` قرار داد؛ در این حالت کد فقط برای شماره‌های سوپرادمین پیکربندی‌شده روی فرم نمایش داده می‌شود. بلافاصله پس از ثبت SMS.ir در تنظیمات، آن را `false` کنید و `docker compose up -d` را اجرا کنید.
