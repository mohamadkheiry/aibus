...
import { API, type Model, request, type UserKey } from './api'
...
const gatewayPath=(mode:PlaygroundMode)=>mode==='tts'?'/v1/audio/speech':mode==='stt'?'/v1/audio/transcriptions':mode==='realtime'?'/v1/realtime':'/v1/chat/completions'
const serviceNames:Record<string,string>={chat:'متن و چت',audio_understanding:'درک صوت',speech_to_text:'صوت به متن',text_to_speech:'متن به صوت',speech_to_speech:'صوت به صوت',realtime_translation:'ترجمه هم‌زمان',translation:'ترجمه و دوبله',voice_clone:'شبیه‌سازی صدا',voice_design:'طراحی صدا'}

const initialPayload=(model:Model,mode:PlaygroundMode)=>{
  const fallback:Record<string,unknown>=mode==='tts'?{model:model.modelId,input:'سلام! این یک آزمایش زنده تولید گفتار در AiBus است.',voice:'alloy',response_format:'mp3'}:mode==='stt'?{model:model.modelId,language:'fa',response_format:'json'}:mode==='realtime'?{model:model.modelId,voice:'alloy',instructions:'به زبان فارسی، کوتاه و طبیعی پاسخ بده.',modalities:['audio','text']}:{model:model.modelId,messages:[{role:'user',content:'در سه جمله کوتاه توضیح بده هوش مصنوعی چگونه بهره‌وری یک کسب‌وکار را افزایش می‌دهد.'}],stream:false}
...
export function Playground({model,close}:{model:Model;close:()=>void}){
  ...
  const origin=API||window.location.origin,endpoint=`${origin}${gatewayPath(mode)}`,shownEndpoint=mode==='realtime'?endpoint.replace(/^http/,'ws'):endpoint
  const recipe=useMemo(()=>createCodeRecipe(language,endpoint,requestJson,mode),[language,endpoint,requestJson,mode])
  ...
  const finishError=(error:unknown)=>{const message=error instanceof Error?error.message:String(error);setOutput(JSON.stringify({error:{message}},null,2));toast.error(message)}
  ...
  const run=async()=>{
    ...
    try{
      ...
      if(mode==='stt'){
        ...
        const res=await fetch(endpoint,{method:'POST',headers:{Authorization:`Bearer ${key}`},body:form}),text=await res.text();let data:unknown;try{data=JSON.parse(text)}catch{data={raw:text}}
        setOutput(JSON.stringify(data,null,2));setMeta({status:res.status,latency:Math.round(performance.now()-started)});if(!res.ok)throw new Error((data as {error?:{message?:string}})?.error?.message||`خطای HTTP ${res.status}`);toast.success('رونویسی صوت دریافت شد')
      }else{
        ...
        if(!res.ok){const text=await res.text();throw new Error(text||`خطای HTTP ${res.status}`)}
        if(mode==='tts'){
  ...
  const startLive=async()=>{
    ...
    try{
      ...
      ws.onopen=()=>{setLive('connected');ws.send(JSON.stringify({type:'session.update',session:{modalities:['audio','text'],voice:String(body.voice||'alloy'),instructions:String(body.instructions||'به زبان فارسی پاسخ بده.'),input_audio_format:'pcm16',output_audio_format:'pcm16',input_audio_transcription:{model:'whisper-1'},turn_detection:{type:'server_vad'}}}));const input=context.createMediaStreamSource(stream),node=context.createScriptProcessor(4096,1,1),silent=context.createGain();silent.gain.value=0;input.connect(node);node.connect(silent);silent.connect(context.destination);sourceNode.current=input;processor.current=node;node.onaudioprocess=e=>{if(ws.readyState===WebSocket.OPEN)ws.send(JSON.stringify({type:'input_audio_buffer.append',audio:pcm16Base64(e.inputBuffer.getChannelData(0),context.sampleRate)}))}}
      ws.onmessage=event=>{try{const data=JSON.parse(String(event.data)) as Record<string,unknown>,type=String(data.type||'event');setEvents(items=>[...items.slice(-79),`${new Date().toLocaleTimeString('fa-IR')} · ${type}`]);const delta=typeof data.delta==='string'?data.delta:'',completed=typeof data.transcript==='string'?data.transcript:'';if(type.includes('transcript')&&delta)setTranscript(value=>value+delta);else if(type.includes('transcript')&&completed)setTranscript(value=>`${value}${value?'\n':''}${completed}`);if((type==='response.audio.delta'||type==='response.output_audio.delta')&&delta)playPcm(delta);if(type==='error')toast.error(String((data.error as {message?:string})?.message||'خطای ارتباط زنده'))}catch{setEvents(items=>[...items.slice(-79),'پیام باینری/ناشناخته دریافت شد'])}}
      ws.onerror=()=>{toast.error('ارتباط WebSocket برقرار نشد. کلید و تنظیمات Provider را بررسی کنید.');stopLive()};ws.onclose=()=>stopLive()
    }catch(error){stopLive();toast.error(error instanceof Error?error.message:'شروع مکالمه زنده ممکن نشد.')}
  }
