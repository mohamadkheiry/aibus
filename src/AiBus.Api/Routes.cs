...
public static class Routes
{
    ...
    private static void MapUser(WebApplication app)
    {
        ...
        api.MapGet("/dashboard", async (System.Security.Claims.ClaimsPrincipal p, AppDbContext db, [FromQuery] DateTime? from, [FromQuery] DateTime? to, CancellationToken ct) =>
        {
            ...
            var totals = await usage.GroupBy(_ => 1).Select(g => new { requests = g.Count(), inputTokens = g.Sum(x => x.InputTokens), outputTokens = g.Sum(x => x.OutputTokens), costUsd = g.Sum(x => x.CostUsd), avgLatencyMs = g.Average(x => x.DurationMs) }).FirstOrDefaultAsync(ct);
            var timelineRaw = await usage.GroupBy(x => x.CreatedAtUtc.Date).Select(g => new { date = g.Key, requests = g.Count(), tokens = g.Sum(x => x.InputTokens + x.OutputTokens), costUsd = g.Sum(x => x.CostUsd) }).OrderBy(x => x.date).ToListAsync(ct);
            var byModel = await usage.GroupBy(x => x.ModelName).Select(g => new { model = g.Key, requests = g.Count(), tokens = g.Sum(x => x.InputTokens + x.OutputTokens), costUsd = g.Sum(x => x.CostUsd) }).OrderByDescending(x => x.tokens).Take(12).ToListAsync(ct);
            return Results.Ok(new { walletUsd = user.WalletUsd, walletIrr = user.WalletUsd * decimal.Parse((await db.Settings.FindAsync(["currency.usd_irr"], ct))?.Value ?? "0", CultureInfo.InvariantCulture), totals = totals ?? new { requests = 0, inputTokens = 0L, outputTokens = 0L, costUsd = 0m, avgLatencyMs = 0d }, timeline = timelineRaw, byModel });
        });
    ...
    private static void MapAdmin(WebApplication app)
    {
        ...
        admin.MapGet("/users", async (AppDbContext db, [FromQuery] string? search, [FromQuery] string? sort, CancellationToken ct) => { var q = db.Users.AsNoTracking().Include(x => x.ApiKeys).AsQueryable(); if (!string.IsNullOrWhiteSpace(search)) q = q.Where(x => x.Mobile.Contains(search) || x.DisplayName.Contains(search)); q = sort switch { "wallet" => q.OrderByDescending(x => x.WalletUsd), "requests" => q.OrderByDescending(x => x.ApiKeys.Sum(k => k.RequestCount)), _ => q.OrderByDescending(x => x.CreatedAtUtc) }; return Results.Ok(await q.Select(x => new { x.Id, x.Mobile, x.DisplayName, x.Role, x.IsSuspended, x.WalletUsd, x.CreatedAtUtc, x.LastSeenAtUtc, apiKeyCount = x.ApiKeys.Count, requests = x.ApiKeys.Sum(k => k.RequestCount), spentUsd = x.ApiKeys.Sum(k => k.SpentUsd) }).ToListAsync(ct)); });
        ...
        admin.MapGet("/dashboard", async (AppDbContext db, CancellationToken ct) =>
        {
            ...
            var timeline = await usages.Where(x => x.CreatedAtUtc >= month).GroupBy(x => x.CreatedAtUtc.Date).Select(g => new { date = g.Key, requests = g.Count(), tokens = g.Sum(x => x.InputTokens + x.OutputTokens), revenueUsd = g.Sum(x => x.CostUsd) }).OrderBy(x => x.date).ToListAsync(ct);
            var models = await usages.Where(x => x.CreatedAtUtc >= month).GroupBy(x => x.ModelName).Select(g => new { model = g.Key, tokens = g.Sum(x => x.InputTokens + x.OutputTokens), requests = g.Count(), revenueUsd = g.Sum(x => x.CostUsd) }).OrderByDescending(x => x.tokens).Take(10).ToListAsync(ct);
            ...
            return Results.Ok(new { users = await db.Users.CountAsync(ct), online = await db.Users.CountAsync(x => x.LastSeenAtUtc >= now.AddMinutes(-5), ct), walletLiabilityUsd = await db.Users.SumAsync(x => x.WalletUsd, ct), todayRequests = await usages.CountAsync(x => x.CreatedAtUtc >= today, ct), todayTokens = await usages.Where(x => x.CreatedAtUtc >= today).SumAsync(x => x.InputTokens + x.OutputTokens, ct), todayRevenueUsd = await usages.Where(x => x.CreatedAtUtc >= today).SumAsync(x => x.CostUsd, ct), uniqueToday = await visits.Where(x => x.CreatedAtUtc >= today).Select(x => x.VisitorId).Distinct().CountAsync(ct), visitsToday = await visits.CountAsync(x => x.CreatedAtUtc >= today, ct), totalVisits = await visits.CountAsync(ct), timeline, models, lowKeys });
        });
